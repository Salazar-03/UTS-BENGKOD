                  ...
                  <div class="card">
                      <div class="card-body">
                        ...
                        <div class="table-responsive">
                            ...
                            <tbody>
                                <?php if($pasien){ $x=1; foreach($pasien as $pasiens){?>
                                <tr>
                                    <td><?=$x++;?></td>
                                    <td><?=$pasiens['nama']?></td>
                                    ...
                                    <td><?=$pasiens['no_hp']?></td>
                                    <td>
                                        <button type="button" class="btn btn-warning ubahData" data-bs-toggle="modal" data-bs-target="#ubahDataModal" data-id="<?=$pasiens['id']?>">Ubah</button>
                                        <button type="button" class="btn btn-danger hapusData" data-id='<?=$pasiens['id']?>'>Hapus</button>
                                    </td>
                                </tr>
                                <?php }}?>
                            </tbody>
  ...
  <!-- container-scroller -->
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formTambahData">
                        ...
                        <div id="errorNamaPasien" class="text-danger"></div>
                        ...
                        <div id="errorAlamatPasien" class="text-danger"></div>

                        <label for="nomorHP" class="form-label">No Handphone Pasien</label>
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
    ...
    <div class="modal fade" id="ubahDataModal" tabindex="-1" aria-labelledby="ubahDataModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ubahDataModalLabel">Ubah Data Pasien</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formUbahData">
                        <input type="hidden" class="form-control" id="id_pasien" name="id_pasien" required>
                        <label for="namaPasienUpdate" class="form-label">Nama Pasien</label>
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <div class="input-group-text">@</div>
                            </div>
                            <input type="text" class="form-control" id="namaPasienUpdate" name="namaPasienUpdate" required>
                        </div>
                        <div id="errorNamaPasienUpdate" class="text-danger"></div>

                        <label for="alamatPasienUpdate" class="form-label">Alamat Pasien</label>
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <div class="input-group-text"><i class="mdi mdi-home"></i></div>
                            </div>
                            <input type="text" class="form-control" id="alamatPasienUpdate" name="alamatPasienUpdate" required>
                        </div>
                        <div id="errorAlamatPasienUpdate" class="text-danger"></div>

                        <label for="nomorHPUpdate" class="form-label">No Handphone Pasien</label>
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <div class="input-group-text"><i class="mdi mdi-cellphone"></i></div>
                            </div>
  ...
  <!-- End custom js for this page-->
    <script>
      function simpanData() {
        ...
        $('#errorNamaPasien').text('');
        $('#errorAlamatPasien').text('');
        $('#errorNomorHP').text('');

        $.ajax({
            url: '<?php echo site_url("welcome/proses_data_pasien"); ?>',
            type: 'POST',
            ...
            success: function(response) {
                if (response.status === 'error') {
                    ...
                    $('#errorNamaPasien').text(response.errors.namaPasien);
                    $('#errorAlamatPasien').text(response.errors.alamatPasien);
      ...
      function updateData() {
        ...
        $.ajax({
          url: '<?php echo site_url("welcome/proses_update_pasien"); ?>',
          type: 'POST',
          ...
          success: function(response) {
            if (response.status === 'error') {
              // Jika terdapat error, tampilkan di form
              $('#errorNamaPasienUpdate').text(response.errors.namaPasienUpdate);
              $('#errorAlamatPasienUpdate').text(response.errors.alamatPasienUpdate);
              $('#errorNomorHPUpdate').text(response.errors.nomorHPUpdate);
              localStorage.setItem('failedUpdate', 'Gagal mengupdate data pasien.');
            } else if (response.status === 'success') {
              // Jika berhasil, tutup modal dan tampilkan pesan sukses
              $('#ubahDataModal').modal('hide');
              localStorage.setItem('successUpdate', 'Berhasil mengupdate data pasien.');
              location.reload();
            }
          }
        });
      ...
      $(document).ready(function() {
          ...
          $('.ubahData').click(function(event) {
            ...
            $.ajax({
              url: '<?= base_url("welcome/get_data_pasien") ?>',
              type: 'POST',
              ...
              success: function(data) {
                if (data) {
                  ...
                  $('#formUbahData input[name="id_pasien"]').val(data.id);
                  $('#formUbahData input[name="namaPasienUpdate"]').val(data.nama);
                  $('#formUbahData input[name="alamatPasienUpdate"]').val(data.alamat);
          ...
          $('.hapusData').click(function(event) {
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      $.ajax({
                          url: '<?= base_url("welcome/delete_data_pasien") ?>',
                          type: 'POST',
                          data: { id_pasien: id_pasien },
                          dataType: 'json',
                          success: function(response) {
                              if (response.status === 'success') {
                                  ...
                                  localStorage.setItem('successDelete', 'Berhasil menghapus data pasien.');
                                  location.reload(); // Refresh halaman
                              } else {
                                  $('#failedDelete').text('Gagal menghapus data pasien');
                              }
                          },
                          error: function() {
                              Swal.fire('Error', 'Terjadi kesalahan saat mengirim data', 'error');
